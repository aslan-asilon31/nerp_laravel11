...
class DepartmentController extends Controller
{
    public function index()
    {
        ...
        $departments = RoleMaster::where('parent_id', null)
        ->get();
        // Mengambil divisi untuk setiap departemen
        foreach ($departments as $department) {
            $department->divisions = RoleMaster::where('parent_id', $department->id)->get();
        }
        $totalDivisions = RoleMaster::whereIn('parent_id', $departments->pluck('id'))->count();
        return view('service.department', compact('departments','categories','totalDivisions','orders','company_now','outlets','companies'));
    }
