...
class GeneralLedgerController extends Controller
{
    public function index()
    {
        ...
        $general_ledger_counts = GeneralJournal::orderBy('id', 'asc');
        $general_ledger_members = GeneralJournal::selectRaw('
                ...
                ->get(); 
        $general_ledgers = GeneralJournal::selectRaw('
                SUBSTRING_INDEX(account_code, ".", 1) AS group_code,
                id,
                account_code,
                account_name,
            ...
            $total_debit_all = $general_ledgers->sum('debit');
            $total_credit_all = $general_ledgers->sum('credit');
            
            // Debug output
            // dd($total_debit_all, $total_credit_all);


        return view('accounting.general_ledger', compact('total_debit_all','total_credit_all','general_ledger_members','general_ledger_counts','general_ledgers','categories','companies','outlets','orders','company_now','workspace'));
    }
}